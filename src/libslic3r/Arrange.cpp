        ...
        std::function<bool()>         stopfn)
{
    ...
    if (params.wear_leveling) {
		printf("Wear leveling is enabled.\n");
		auto bb = sl::boundingBox(corrected_bin);
		auto center = bb.center();
		coord_t c1 = center(0,0);
		coord_t c2 = center(1,0);
		printf("Bin center:\t%d, %d\n", c1, c2);
		auto binmax = bb.maxCorner();
		auto binmin = bb.minCorner();
		printf("Bin max:\t%d, %d\n", binmax(0,0), binmax(1,0));
		printf("Bin min:\t%d, %d\n", binmin(0,0), binmin(1,0));
		Point itmsMin(center);
		Point itmsMax(center);
		for (auto &itm : shapes) {
			auto ibb = itm.boundingBox();
			auto ibbMax = ibb.maxCorner();
			auto ibbMin = ibb.minCorner();
			printf("Item max:\t%d, %d\n", ibbMax(0,0), ibbMax(1,0));
			printf("Item min:\t%d, %d\n", ibbMin(0,0), ibbMin(1,0));
			if (itmsMin(0,0) > ibbMin(0,0))
				itmsMin(0,0) = ibbMin(0,0);
			if (itmsMin(1,0) > ibbMin(1,0))
				itmsMin(1,0) = ibbMin(1,0);
			if (itmsMax(0,0) < ibbMax(0,0))
				itmsMax(0,0) = ibbMax(0,0);
			if (itmsMax(1,0) < ibbMax(1,0))
				itmsMax(1,0) = ibbMax(1,0);
		}
		printf("Group min:\t%d, %d\n", itmsMin(0,0), itmsMin(1,0));
		printf("Group max:\t%d, %d\n", itmsMax(0,0), itmsMax(1,0));

		coord_t distLeft = binmax(0,0) - itmsMax(0,0);
		coord_t distTop = binmax(1,0) - itmsMax(1,0);
		coord_t distRight = itmsMin(0,0) - binmin(0,0);
		coord_t distBottom = itmsMin(1,0) - binmin(1,0);
		coord_t offsetX = (rand() % (distLeft + distRight)) - distLeft;
		coord_t offsetY = (rand() % (distTop + distBottom)) - distBottom;
		printf("Rand: %d, %d\n", offsetX, offsetY);

		for (auto &itm : shapes) {
			itm.translate({offsetX, offsetY});
		}
    }
